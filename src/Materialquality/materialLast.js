...
export class MaterialLastScreen extends Component {
  _isMounted = false;
  ...
  getData() {
    ...
    storage.sync = {
      ...
      async materiallast(params) {
        return (
          fetch("http://phlapp.drcmp.org/api/getmaterialshousegoodbadimages/"+_this.state.curId, {
            method: "GET",
            headers: {
    ...
    return (
      <Container>
        <Header style={{backgroundColor:'#9ec54d'}}>
          ...
          <Body>
            <Title>{t('material_Photo:back')}</Title>
          </Body>
          <Right />
        </Header>
        ...
        {this.state.safetyData == null ? (
          ...
          <View style={{ flex: 1,backgroundColor:'#eeeeee',marginTop:10 }}>
            <CardStack
          ...
          >
            <Text style={styles.customBtnText}>{t('material_Photo:quality')}</Text>
          </Button>
          <Button
            style={styles.customBtnBG}
              ...
              {this.state.safetyData.map((item, index) => {
                ...
                return (
                  <Card style={[styles.card, styles.card1]}>
                    <View >
                      <View style={{backgroundColor:'#6383a8'}}>
...
export default withNamespaces(["material_Photo", "common"], { wait: true })(
  MaterialLastScreen
);
